...
% hold on;
for i=1:30
   d_UWB_3(R_UWB_3_U(i))= d_UWB_3(R_UWB_3_U(i))+rand(1)+2;
end
...
% hold on;
for i=1:30
   d_UWB_4(R_UWB_4_U(i))= d_UWB_4(R_UWB_4_U(i))+2+rand(1);
end
...
%     计算卡尔曼增益
    ...
    msd=sqrt((X_est(k,1)-X_est(k-1,1))^2+(X_est(k,2)-X_est(k-1,2))^2);
    %计算前一个时刻基站到移动节点的距离
    for g=1:6
        d_t_1(g)=sqrt((X_est(k-1,1)-bs(g,1))^2+(X_est(k-1,2)-bs(g,2))^2);%%---????上一个时刻的位置应该用融合算出来的位置
    end
    %计算残差
    residual=ones(6,1);
    for g=1:6                          %-----此处假设有6个基站

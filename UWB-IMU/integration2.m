...
% hold on;
for i=1:30
   d_UWB_3(R_UWB_3_U(i))= d_UWB_3(R_UWB_3_U(i))+rand(1)+1;
end
...
% hold on;
for i=1:30
   d_UWB_4(R_UWB_4_U(i))= d_UWB_4(R_UWB_4_U(i))+1+rand(1);
end
...
%       residual(g)=abs(msd-abs(d(k-1,g)-d(k,g))-Threshold)*k_r;   %Threshold=-1待定  ------   k_r为残差到构造观测矩阵的一个调节系数   
        ...
        if(R_R_R(k,g)<-0.05) 
            residual(g)=abs( R_R_R(k,g)+0.05)*1000;
        end
    end
    %计算融合时需要的测量噪声矩阵

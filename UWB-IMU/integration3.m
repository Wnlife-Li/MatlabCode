...
% hold on;
for i=1:30
   d_UWB_3(R_UWB_3_U(i))= d_UWB_3(R_UWB_3_U(i))+rand(1)+0.5;
end
...
% hold on;
for i=1:30
   d_UWB_4(R_UWB_4_U(i))= d_UWB_4(R_UWB_4_U(i))+0.5+rand(1);
end
...
%     de=diag([residual(1); residual(2); residual(3); residual(4); residual(5); residual(6); 1; 1]);%融合噪声的矩阵的残差系数
    ...
    %%%%-----NLOS检测
    MM=1.96*sqrt(det(HH_R(1:6,:)*Pkk_R*HH_R(1:6,:)'+R_F(1:6,1:6)));
    %%%------%%%  
    r_d=d(k,:)' - y_R_F;
    residual=ones(6,1);
    for i=1:6
        if(r_d(i)<-MM||r_d(i)>MM)
            residual(i)=1000;  %%计算残差
        end
    end
    de=diag([residual(1); residual(2); residual(3); residual(4); residual(5); residual(6); 1; 1]);%融合噪声的矩阵的残差系数
    R_E=R_F*de;   %----  融合噪声的矩阵

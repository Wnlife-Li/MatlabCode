...
clear all;
%%%%%%%%%%%%%%-------------基于UWB和IMU的室内定位融合算法--------------%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%===============【自己的方法   加了三角形算法对比  模拟的作用范围是小范围  噪声使用了awg函数】===============%%%%%%%%%%%%%%%%%%%%%%%
%----1.模拟惯性导航的数据
%%%---1.1 6个基站的坐标
x1=[1 1];
x2=[1 9];
...
%         if(R_R_R(k,g)<-0.05) 
%             residual(g)=abs( R_R_R(k,g)+0.05)*60;
%         end
%     end
    %计算融合时需要的测量噪声矩阵
...
plot(1,9,'rs','MarkerFaceColor','r','MarkerSize',12);
plot(5,9,'rs','MarkerFaceColor','r','MarkerSize',12);
plot(9,9,'rs','MarkerFaceColor','r','MarkerSize',12);
plot(9,1,'rs','MarkerFaceColor','r','MarkerSize',12);
plot(5,1,'rs','MarkerFaceColor','r','MarkerSize',12);
axis([0 10 0 10]);
xlabel('东向距离/m'); 
ylabel('北向距离/m');
